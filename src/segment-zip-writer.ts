...
function stamp(date = new Date()) { const y = Math.max(1980, date.getFullYear()); return { time: (date.getHours() << 11) | (date.getMinutes() << 5) | Math.floor(date.getSeconds() / 2), date: ((y - 1980) << 9) | ((date.getMonth() + 1) << 5) | date.getDate() }; }
...
export async function createSegmentZipWriter(exportJobId: string) {
  ...
  return {
    path,
    async add(filename: string, bytes: Uint8Array) {
      ...
      const t = stamp(); const localOffset = offset; const crc = crc32(bytes);
