...
export async function processSpeakerDiarization(job:Job<SpeakerDiarizationJobData>){
  ...
  try{return await runWithLockHeartbeat(job,async signal=>{
    ...
    for(const segment of source){const words=segment.aai_word_timings?.length?segment.aai_word_timings:[{text:segment.source_text,start_ms:segment.start_ms,end_ms:segment.end_ms,confidence:segment.confidence}];const mapped=words.map(w=>{const t=best(turns,Number(w.start_ms),Number(w.end_ms),cursor);if(!t)unresolved++;else if((segment.speaker_label||'')!==label(t.speaker,clusters.indexOf(t.speaker)))reassigned++;return {...w,cluster:t?.speaker||null};});for(let i=0;i<mapped.length;i++)mapped[i].cluster ||= mapped[i-1]?.cluster||mapped[i+1]?.cluster||clusters[0];const groups:{cluster:string;words:Word[]}[]=[];for(const w of mapped){const c=String(w.cluster);const last=groups.at(-1);last?.cluster===c?last.words.push(w):groups.push({cluster:c,words:[w]});}splits+=Math.max(0,groups.length-1);for(const g of groups){const sp=speakers[g.cluster],cs=g.words.map(w=>w.confidence).filter((n):n is number=>n!=null),start=Number(g.words[0].start_ms),end=Number(g.words.at(-1)!.end_ms);counts[g.cluster]=(counts[g.cluster]||0)+1;output.push({sequence_index:output.length,start_ms:start,end_ms:end,tc_in:tc(start,prep.project.frame_rate||25),tc_out:tc(end,prep.project.frame_rate||25),speaker_id:sp.id,speaker_label:sp.label,speaker_color:sp.color,source_text:join(g.words)||segment.source_text,source_text_status:segment.source_text_status||'machine',confidence:cs.length?cs.reduce((a,b)=>a+b,0)/cs.length:segment.confidence,avg_word_confidence:cs.length?cs.reduce((a,b)=>a+b,0)/cs.length:segment.avg_word_confidence,aai_word_timings:g.words.map(({cluster:_c,...w})=>w),provider_name:segment.provider_name,version_number:segment.version_number||1,...(segment.consensus_run_id?{consensus_run_id:segment.consensus_run_id}:{}),...(segment.consensus_word_sources?.length?{consensus_word_sources:segment.consensus_word_sources.filter(w=>Number(w.end_ms)>start&&Number(w.start_ms)<end)}:{}),...(segment.is_music?{is_music:true,music_source:segment.music_source,music_context:segment.music_context||'foreground'}:{})});}}
    for(let i=0;i<output.length;i+=200){await call('stage_segments',{segments:output.slice(i,i+200)},signal);await call('heartbeat',{progress_pct:Math.min(96,82+Math.floor(i/output.length*14))},signal);}
    const cs=plans.map(p=>p.confidence).filter((n):n is number=>n!==null),avg=cs.length?cs.reduce((a,b)=>a+b,0)/cs.length:null;await call('finalize',{raw_result_key:prep.raw_result_key,provider_job_id:providerId,source_segment_count:source.length,output_segment_count:output.length,detected_speaker_count:clusters.length,split_count:splits,reassigned_word_count:reassigned,unresolved_word_count:unresolved,average_confidence:avg,speaker_counts:counts},signal);
