...
provider_boundary_start_ms?:number|null; provider_boundary_end_ms?:number|null; boundary_source?:string; consensus_run_id?:string; consensus_word_sources?:Array<{start_ms?:number;end_ms?:number;[key:string]:unknown}>; is_music?:boolean; music_source?:string; music_context?:string; source_text_approved?:boolean; source_text_approved_by?:string; source_text_approved_at?:string };
...
export async function processSpeakerDiarization(job:Job<SpeakerDiarizationJobData>){
  ...
  try{return await runWithLockHeartbeat(job,async signal=>{
    ...
    // Approved scripts use physical file rows only as temporary ingestion blocks.
    // Their immutable WORD STREAM is aligned globally; measured pauses and pyannote
    // turns define the final localization segments. This allows one final segment
    // to span several source lines without permitting a single word to change.
    const approvedRows=source.filter(segment=>segment.provider_name==='approved_script_import');
    const approvedScriptMode=approvedRows.length>0;
    if(approvedScriptMode&&approvedRows.length!==source.length)throw new Error('approved_script_mixed_generation: approved and ordinary transcript rows cannot be refined together');
    const materializeWords=(segment:Segment)=>{
      const existing=segment.aai_word_timings||[];
      if(existing.length)return existing;
      const tokens=String(segment.source_text||'').trim().split(/\s+/).filter(Boolean);
      const span=Math.max(tokens.length,Number(segment.end_ms)-Number(segment.start_ms));
      return tokens.map((text,index)=>({text,start_ms:Number(segment.start_ms)+(span*index/tokens.length),end_ms:Number(segment.start_ms)+(span*(index+1)/tokens.length),confidence:1}));
    };
    const approvedWords=approvedScriptMode?source.flatMap(materializeWords):[];
    const processingSource:Segment[]=approvedScriptMode?[{...source[0],id:'approved-script',sequence_index:0,start_ms:0,end_ms:Number(prep.project.duration_ms||source.at(-1)?.end_ms||1),source_text:join(approvedWords),aai_word_timings:approvedWords,provider_name:'approved_script_import'}]:source;
    const alignmentInput=processingSource.flatMap(segment=>{const words=segment.aai_word_timings||[];if(!words.length&&!segment.is_music&&!textIsAuthoritative(segment))throw new Error(`Missing provider word timings for segment ${segment.id}`);return words.map((word,index)=>({key:`${segment.id}:${index}`,text:String(word.text||'').trim(),provider_start_ms:Number(word.start_ms),provider_end_ms:Number(word.end_ms)})).filter(word=>word.text);});
    if(!alignmentInput.length)throw new Error('No words available for forced alignment');
    ...
    const alignment=await withPhase('aligning_words',62,`Aligning ${alignmentInput.length} words against the source audio (${source.length} lines). This is the longest stage and scales with programme length.`,signal,
      ()=>alignTranscript({requestId:`${request_id}:${run_id}`,audioUrl:prep.source_url,languageCode:languageResolution.code,words:alignmentInput,signal,alignmentMode:approvedScriptMode?'untimed_script':'provider_timed'}));
    ...
    const providerByKey=new Map(approvedScriptMode?alignment.words.map(word=>[word.key,{start_ms:Number(word.start_ms),end_ms:Number(word.end_ms)}]):alignmentInput.map(word=>[word.key,{start_ms:Number(word.provider_start_ms),end_ms:Number(word.provider_end_ms)}]));
    ...
    for(const segment of processingSource){const providerWords=segment.aai_word_timings||[];
...
return {...word,start_ms:aligned.start_ms,end_ms:aligned.end_ms,alignment_confidence:aligned.confidence,onset_reconstructed:aligned.onset_reconstructed===true,onset_absorbed_ms:Number(aligned.onset_absorbed_ms||0),capture_restored:aligned.capture_restored===true,capture_divergence_ms:Number(aligned.capture_divergence_ms||0),provider_timing_rejected:aligned.provider_timing_rejected===true,provider_timing_rejected_ms:Number(aligned.provider_timing_rejected_ms||0),alignment_collapsed:aligned.alignment_collapsed===true,unresolved:aligned.unresolved===true,unresolved_reason:String(aligned.unresolved_reason||''),unresolved_cleared:aligned.unresolved_cleared===true,unresolved_cleared_reason:String(aligned.unresolved_cleared_reason||''),unresolved_prior_reason:String(aligned.unresolved_prior_reason||''),search_window_exhausted:aligned.search_window_exhausted===true,arbitration_reason:String(aligned.arbitration_reason||''),arbitration_ceiling_ms:Number(aligned.arbitration_ceiling_ms||0),arbitration_provider_duration_ms:aligned.arbitration_provider_duration_ms??null,arbitration_aligned_duration_ms:Number(aligned.arbitration_aligned_duration_ms||0),prior_start_ms:aligned.prior_start_ms??null,prior_end_ms:aligned.prior_end_ms??null,expansion_lead_ms:aligned.expansion_lead_ms??null,expansion_trail_ms:aligned.expansion_trail_ms??null,alignment_pass:aligned.alignment_pass??null,chunk_index:aligned.chunk_index??null,provider_word:word};});const mapped:any[]=acousticWords.map(w=>{const t=best(turns,Number(w.start_ms),Number(w.end_ms),cursor);if(!t)unresolved++;else if((segment.speaker_label||'')!==label(t.speaker,clusters.indexOf(t.speaker)))reassigned++;return {...w,cluster:t?.speaker||null,speaker_unresolved:!t};});
      for(let i=0;i<mapped.length;i++)mapped[i].cluster ||= mapped[i-1]?.cluster||mapped[i+1]?.cluster||clusters[0];const groups:{cluster:string;words:any[]}[]=approvedScriptMode?segmentApprovedScriptWords(mapped):[];if(!approvedScriptMode)for(const w of mapped){const c=String(w.cluster);const last=groups.at(-1);last?.cluster===c?last.words.push(w):groups.push({cluster:c,words:[w]});}// Refuse to divide an authoritative row: there is no way to split
...
unresolvedEvidence=g.words.map((w,wordIndex)=>({w,wordIndex})).filter(({w})=>w.unresolved===true).slice(0,100).map(({w,wordIndex})=>({text:String(w.text||''),word_index:wordIndex,provider_start_ms:approvedScriptMode?Number(w.start_ms):Number(w.provider_word.start_ms),provider_end_ms:approvedScriptMode?Number(w.end_ms):Number(w.provider_word.end_ms),aligned_start_ms:w.prior_start_ms??Number(w.start_ms),aligned_end_ms:w.prior_end_ms??Number(w.end_ms),display_start_ms:Number(w.start_ms),display_end_ms:Number(w.end_ms),unresolved_reason:String(w.unresolved_reason||(w.alignment_collapsed?'alignment_collapse_no_usable_window':'unresolved_without_reported_reason')),search_window_exhausted:w.search_window_exhausted===true,arbitration_reason:String(w.arbitration_reason||''),arbitration_ceiling_ms:Number(w.arbitration_ceiling_ms||0),arbitration_provider_duration_ms:w.arbitration_provider_duration_ms??null,arbitration_aligned_duration_ms:Number(w.arbitration_aligned_duration_ms||0),alignment_pass:w.alignment_pass??null,expansion_lead_ms:w.expansion_lead_ms??null,expansion_trail_ms:w.expansion_trail_ms??null,chunk_index:w.chunk_index??null})),maxShift=Math.max(...alignmentWords.map(w=>Math.max(Math.abs(w.start_ms-w.provider_start_ms),Math.abs(w.end_ms-w.provider_end_ms))));counts[g.cluster]=(counts[g.cluster]||0)+1;output.push({sequence_index:output.length,start_ms:start,end_ms:end,tc_in:tc(start,prep.project.frame_rate||25),tc_out:tc(end,prep.project.frame_rate||25),speaker_id:sp.id,speaker_label:sp.label,speaker_color:sp.color,source_text:approvedScriptMode?join(g.words):resolveOutputText(segment,join(g.words)),source_text_status:segment.source_text_status||'machine',...(approvedScriptMode?{source_text_approved:true,source_text_approved_text_hash:approvedTextHash(join(g.words)),source_text_approved_by:segment.source_text_approved_by,source_text_approved_at:segment.source_text_approved_at}:{}),confidence:cs.length?cs.reduce((a,b)=>a+b,0)/cs.length:segment.confidence,avg_word_confidence:cs.length?cs.reduce((a,b)=>a+b,0)/cs.length:segment.avg_word_confidence,aai_word_timings:g.words.map(w=>approvedScriptMode?{text:String(w.text||''),start_ms:Number(w.start_ms),end_ms:Number(w.end_ms),confidence:Number(w.alignment_confidence||0)}:w.provider_word),provider_name:segment.provider_name,version_number:segment.version_number||1,_alignment:{status:'verified',model:alignment.model,model_revision:alignment.model_revision,language_code:alignment.language_code,words:alignmentWords,mean_confidence:alignmentScores.reduce((a,b)=>a+b,0)/alignmentScores.length,max_provider_shift_ms:maxShift,raw_result_key:prep.alignment_result_key,unresolved_words:unresolvedEvidence,unresolved_word_count:g.words.filter(w=>w.unresolved===true).length,
...
...(approvedScriptMode?{provider_boundary_start_ms:null,provider_boundary_end_ms:null,boundary_source:''}:resolveProviderBoundary(segment,{isSplit:outputGroups.length!==1})),
...
...(()=>{const rec=g.words.filter(w=>w.onset_reconstructed===true);return rec.length?{onset_reconstructed:true,onset_absorbed_ms:Math.max(...rec.map(w=>Number(w.onset_absorbed_ms||0)))}:{};})(),...(()=>{const res=g.words.filter(w=>w.capture_restored===true);return res.length?{capture_restored:true,capture_restored_ms:Math.max(...res.map(w=>Number(w.capture_divergence_ms||0)))}:{};})(),...(()=>{const rej=g.words.filter(w=>w.provider_timing_rejected===true);return rej.length?{provider_timing_rejected:true,provider_timing_rejected_ms:Math.max(...rej.map(w=>Number(w.provider_timing_rejected_ms||0)))}:{};})(),...(g.words.some(w=>w.alignment_collapsed===true)?{alignment_collapsed:true}:{}),...(segment.consensus_run_id?{consensus_run_id:segment.consensus_run_id}:{}),...(segment.consensus_word_sources?.length?{consensus_word_sources:segment.consensus_word_sources.filter(w=>Number(w.end_ms)>start&&Number(w.start_ms)<end)}:{}),...(segment.is_music?{is_music:true,music_source:segment.music_source,music_context:segment.music_context||'foreground'}:{})});}}
    ...
    if(approvedScriptMode){const expected=join(approvedWords);const delivered=output.map(row=>String(row.source_text||'')).join(' ').replace(/\s+([,.;:!?])/g,'$1').replace(/\s+/g,' ').trim();if(delivered!==expected)throw new Error(`approved_script_text_conservation_failed: aligned output hash ${approvedTextHash(delivered)} does not match imported word-stream hash ${approvedTextHash(expected)}`);}
