...
import { auditTimelineIntegrity, clampAcousticOnsets } from '../timeline-integrity.js';
...
export async function processSpeakerDiarization(job:Job<SpeakerDiarizationJobData>){
  ...
  try{return await runWithLockHeartbeat(job,async signal=>{
    ...
    // The RAW provider response is archived above, untouched. Only the CONSUMED
    // timeline is repaired: pull absorbed onsets forward to a plausible word start
    // so one un-transcribed audio gap can no longer define a segment window.
    const onsetRepair=clampAcousticOnsets(alignment.words);
    ...
    const integrity=auditTimelineIntegrity(output,(ms:number)=>tc(ms,prep.project.frame_rate||25),onsetRepair.report);
