...
interface PhaseStepResponse {
  ...
  audio_job?: {
    ...
    clips: Array<{ url: string; start_ms: number; speaker_id: string }>;
    duration_ms: number;
...
async function callMixFinal(opts: {
  railwayUrl: string;
  railwayKey: string;
  clips: Array<{ url: string; start_ms: number; gain_db?: number }>;
  durationMs: number;
    ...
    for (let i = 0; i < MAX_PHASE_ITERATIONS; i++) {
      ...
      if (step.action === 'render_audio') {
        ...
        if (aj.mode === 'per_speaker') {
          ...
          const groups: Record<string, Array<{ url: string; start_ms: number }>> = {};
          for (const c of aj.clips) {
            (groups[c.speaker_id] ||= []).push({ url: c.url, start_ms: c.start_ms });
          }
          ...
          const mixBytes = await callMixFinal({
            railwayUrl, railwayKey,
            clips: aj.clips.map(c => ({ url: c.url, start_ms: c.start_ms, gain_db: dubGain })),
          ...
          const bytes = await callMixFinal({
            railwayUrl, railwayKey,
            clips: aj.clips.map(c => ({ url: c.url, start_ms: c.start_ms })),
            durationMs: aj.duration_ms,
            meTrackUrl: aj.me_track_url,
            loudnessTargetLufs: aj.loudness_target_lufs,
          });
