...
export async function invokeBase44Function<T = unknown>(opts: InvokeOpts): Promise<T> {
  ...
  for (let iter = 0; iter < TOTAL_ITERATION_CAP; iter++) {
    ...
    try {
      ...
      // INTENTIONAL STAND-DOWN — checked FIRST, before any retry budget is
      // consulted. This is not a failure to recover from; it is the answer.
      // Retrying it would re-ask a question the platform has already settled,
